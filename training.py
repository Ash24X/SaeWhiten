...
def trainSAE(
    ...
    # Save PCA whitening parameters after training (when they're actually fitted)
    for save_dir, trainer in zip(save_dirs, trainers):
        if save_dir is not None and hasattr(trainer, 'whitener') and trainer.whitener is not None and trainer.whitener.is_fitted:
            pca_params = {
                "whitening_matrix": trainer.whitener.whitening_matrix.cpu(),
                "dewhitening_matrix": trainer.whitener.dewhitening_matrix.cpu(),
                "mean": trainer.whitener.mean.cpu(),
                "activation_dim": trainer.whitener.activation_dim,
                "eps": trainer.whitener.eps
            }
            t.save(pca_params, os.path.join(save_dir, "pca_whitening.pt"))
            print(f"Saved PCA whitening parameters to {os.path.join(save_dir, 'pca_whitening.pt')}")
