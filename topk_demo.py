...
class PCAWhitener:
    ...
    Copy of the PCAWhitener class to avoid import issues.
    """
...
class WhitenedAutoEncoderWrapper:
    """
    Wrapper that automatically whitens inputs and dewhitens outputs for an AutoEncoder.
    """
    def __init__(self, ae, whitener):
        self.ae = ae
        self.whitener = whitener
        # Expose necessary attributes that evaluation code might access
        self.activation_dim = ae.activation_dim
        self.dict_size = ae.dict_size
        
        # Expose other attributes that might be needed
    ...
    def encode(self, x, **kwargs):
        """Whiten input, then encode"""
        if self.whitener is not None and self.whitener.is_fitted:
            x = self.whitener.whiten(x)
        return self.ae.encode(x, **kwargs)
    
    def decode(self, f):
        """Decode and dewhiten output"""
        x_hat = self.ae.decode(f)
        if self.whitener is not None and self.whitener.is_fitted:
            x_hat = self.whitener.dewhiten(x_hat)
        return x_hat
    
    def forward(self, x, output_features=False):
        """Full forward pass with whitening/dewhitening"""
        if self.whitener is not None and self.whitener.is_fitted:
            x_whitened = self.whitener.whiten(x)
            x_hat_whitened, f = self.ae(x_whitened, output_features=True)
    ...
    def __call__(self, x, output_features=False):
        return self.forward(x, output_features)
    
    def to(self, *args, **kwargs):
        """Pass through to underlying autoencoder"""
        self.ae = self.ae.to(*args, **kwargs)
        if self.whitener is not None:
            if hasattr(self.whitener, 'mean') and self.whitener.mean is not None:
                device = None
                dtype = None
                
                # Parse arguments to find device and dtype
                for arg in args:
                    if isinstance(arg, str) and ('cuda' in arg or 'cpu' in arg):
                        device = arg
                    elif isinstance(arg, t.dtype):
                        dtype = arg
                ...
                # Move whitener tensors
                if device is not None:
                    self.whitener.mean = self.whitener.mean.to(device)
                    self.whitener.whitening_matrix = self.whitener.whitening_matrix.to(device)
                    self.whitener.dewhitening_matrix = self.whitener.dewhitening_matrix.to(device)
                if dtype is not None:
                    self.whitener.mean = self.whitener.mean.to(dtype)
                    self.whitener.whitening_matrix = self.whitener.whitening_matrix.to(dtype)
                    self.whitener.dewhitening_matrix = self.whitener.dewhitening_matrix.to(dtype)
        return self
...
def eval_saes(
    ...
    for ae_path in ae_paths:
        ...
        # NEW: Load PCA whitening parameters if they exist
        pca_path = f"{ae_path}/pca_whitening.pt"
        if os.path.exists(pca_path):
            print(f"Loading PCA whitening parameters from {pca_path}")
            pca_params = t.load(pca_path, map_location=device)

            # Create a PCAWhitener instance with loaded parameters
            whitener = PCAWhitener(
                activation_dim=pca_params["activation_dim"],
                eps=pca_params.get("eps", 1e-6),  # Use default if not present
                device=device
            )

            whitener.mean = pca_params["mean"].to(device)
            whitener.whitening_matrix = pca_params["whitening_matrix"].to(device)
            whitener.dewhitening_matrix = pca_params["dewhitening_matrix"].to(device)
            whitener.is_fitted = True

            # Wrap the dictionary
            dictionary = WhitenedAutoEncoderWrapper(dictionary, whitener)
            dictionary = dictionary.to(dtype=model.dtype)
            print("Wrapped SAE with PCA whitening")
        else:
            print(f"No PCA whitening parameters found at {pca_path}, using unwrapped SAE")
